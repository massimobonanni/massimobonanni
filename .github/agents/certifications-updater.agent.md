...
- ONLY update the table rows within each section. DO NOT remove existing certifications that are not present in the transcript. Leave them in the table. Add only new certifications that are present in the transcript but not in the table.
